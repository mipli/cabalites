...
export default class Console {
  ...
  constructor(width: number, height: number, foreground: Core.Color = 0xffffff, background: Core.Color = 0x000000) {
    ...
    this._tint = Core.Utils.buildMatrix<Core.Color>(this.width, this.height, null);
  ...
  setTint(color: Core.Color, x: number, y: number, width: number = 1, height: number = 1) {
    this.setMatrix(this._tint, color, x, y, width, height);
  }
