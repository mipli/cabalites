...
export class TopologyCombinator {
  ...
  initialize(): number[][] {
    ...
    for (var x = 0; x < this.width; x++) {
      for (var y = 0; y < this.height; y++) {
        this.addTopology(new Core.Vector2(x, y));
      }
    }
  ...
  private getEdges(a: number, b: number) {
    const hasTopologyNeighbour = (position: Core.Vector2, topologyId: number) => {
    ...
    for (var x = 0; x < this.width; x++) {
      for (var y = 0; y < this.height; y++) {
        let position = new Core.Vector2(x, y);
  ...
  private addTopology(position: Core.Vector2, topologyId: number = -1) {
    const x = position.x;
    const y = position.y;
    ...
    const neighbours = Map.Utils.getNeighbours(new Core.Vector2(x, y), this.width, this.height, true);
  ...
  private pruneDeadEnd(position: Core.Vector2) {
  ...
  pruneDeadEnds() {
    for (var x = 0; x < this.width; x++) {
      for (var y = 0; y < this.height; y++) {
        if (this.cells[x][y] === 0) {
          this.pruneDeadEnd(new Core.Vector2(x, y));
        }
      }
    }
  }
